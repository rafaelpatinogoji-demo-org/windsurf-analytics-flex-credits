#!/usr/bin/env python3
"""
Team Daily Flex Credits
Fast parallel processing for large teams (thousands of users).
"""
...
def save_results(daily_data, month_name, year):
    """Save results to CSV"""
    sorted_dates = sorted(daily_data.keys())
    current_date = datetime.now().strftime("%Y-%m-%d")
    
    csv_filename = os.path.join(OUTPUT_DIR, f"team_daily_flex_credits_{month_name.lower()}_{year}_{current_date}.csv")
    
    with open(csv_filename, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['event_date', 'date_formatted', 'total_flex_credits', 'total_prompt_credits', 'data_points'])
        writer.writeheader()
    ...
    # Print summary
    print("=" * 100)
    print(f"TEAM FLEX CREDITS - {month_name.upper()} {year}")
    print("=" * 100)
    print(f"\n{'Event Date':<15} {'Date':<25} {'Total Flex Credits':<25} {'Total Prompt Credits'}")
    print("-" * 100)
    ...
    for date in sorted_dates:
        data = daily_data[date]
        date_formatted = format_date_for_display(date)
        print(f"{date:<15} {date_formatted:<25} {data['total_flex_credits']:>20,.2f}     {data['total_prompt_credits']:>20,.2f}")
        total_flex += data['total_flex_credits']
        total_prompt += data['total_prompt_credits']
    
    print("-" * 100)
    print(f"{'TOTAL':<15} {'':<25} {total_flex:>20,.2f}     {total_prompt:>20,.2f}")
    print("=" * 100)
    
    print(f"\n📊 Statistics:")
    print(f"   - Days with data: {len(sorted_dates)}")
    print(f"   - Total flex credits: {total_flex:,.2f}")
    print(f"   - Total prompt credits: {total_prompt:,.2f}")
    if len(sorted_dates) > 0:
        print(f"   - Avg flex credits/day: {total_flex / len(sorted_dates):,.2f}")
    
    days_with_flex = [d for d in sorted_dates if daily_data[d]['total_flex_credits'] > 0]
    if days_with_flex:
        print(f"\n⚠️  Days with flex credits: {len(days_with_flex)}")
    else:
        print(f"\n✅ No flex credits used")


def main():
    parser = argparse.ArgumentParser(
        description='Team Flex Credits - Fast parallel processing',
        epilog='Examples:\n'
               '  python team_daily_flex_credits.py --year 2025 --month 9\n'
    ...
    daily_data = aggregate_by_date(items)
    
    print("💾 Saving results to CSV...")
    save_results(daily_data, month_name, year)
